...
```
[coleta] → [dedup] → [resume+rankeia (Gemini)] → [dashboard + notifica canais]
```
...
| Síntese e ranking com Gemini | [src/summarize.py](src/summarize.py) |
...
2. Em **Settings → Secrets and variables → Actions**, cadastre:
   - Secret `GEMINI_API_KEY` (pegue em https://aistudio.google.com/apikey)
